...
def fetch_db():
    try:
        ...
        query='''select metadev.id,metadev.urn,metadev.ddns,metadev.ip,metadev.port,metadev."videoEncodingInformation",dev."remoteUsername",metadev.rtsp,dev."remoteDeviceSalt",feature."name"
            from "Device" dev
            inner join "DeviceMetaData" metadev on dev.id= metadev."deviceId"
            inner join "Features" feature on dev.id= feature.id
            ;'''
